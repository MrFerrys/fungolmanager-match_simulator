...
def obtener_stats_jugador(player_id, season=2024, league=140):
    ...
    res = requests.get(url, headers=HEADERS).json()
    if res["response"]:
        return res["response"][0]["statistics"][0]
    return None
...
def generar_csv_laliga_completo(salida="la_liga_2024_atributos.csv"):
    ...
    for tm in equipos:
        ...
        for j in jugadores:
            pl = j["player"]
            stats = obtener_stats_jugador(pl["id"])
            if not stats:
                continue
    ...
    print(f"✅ CSV generado con éxito: {salida}")
